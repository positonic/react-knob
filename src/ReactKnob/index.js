...
class LittleFatty extends React.Component {
  ...
  pointermove(event) {
    ...
    var offset =
      (this.state.startPosY -
        event.pageY -
        this.state.startPosX +
        event.pageX) *
      this.state.sensitivity;
    var value =
      this.state.min +
      (((this.state.startVal +
        ((this.state.max - this.state.min) * offset) /
          ((event.shiftKey ? 4 : 1) * 128) -
        this.state.min) /
        this.state.ctlStep) |
        0) *
        this.state.ctlStep;
  ...
  */
  ...
  redraw() {
    ...
    if (this.sprites) {
      var offset =
        (((this.sprites * (this.value - this.min)) / range) | 0) * this.height;
